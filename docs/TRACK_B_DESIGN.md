...
The exact Track B path remains the first demo:

1. Alice creates a personal Agent.
2. The Agent is denied Alice's private document before consent.
3. Alice grants read access; the same Agent succeeds.
4. The Agent attempts to read Bob's private document and receives a backend
   `Access Denied`.
5. The decision view identifies Alice, the Agent, the action, both resources,
   and the different reason codes.

The differentiated product path follows only if time permits:

1. Alpha's coordinator runs an ordered two-Agent task with human approval.
2. An Alpha Agent is denied a Beta resource even when Bob, a member of both
   groups, initiates the request.
3. A group Agent is denied Alice's private data, then succeeds after Alice
   grants it to the current task; the grant expires at task completion.
4. Removing Bob from Alpha prevents further Alpha Agent access.

Automated tests cover forged HTTP identity/Run/task fields, per-human Agent-DM
