...
export class WorkspaceManager {
  ...
  async writeRuntimeTools(runtimePath: string): Promise<void> {
    ...
    const source = [
      ...
      "  process.exit(2);",
      "}",
      "if (!['list', 'read', 'assess', 'disclose'].includes(command)) {",
      "  console.error(JSON.stringify({ error: 'Usage: vault.mjs list | read|assess|disclose --owner <username> [--title <title>]' }));",
      "  process.exit(2);",
      "}",
      ...
      "  if (ownerUsername && (title || command === 'disclose')) {",
  ...
  private async writeSharedInstructions(
    ...
    const content = [
        ...
        "- Requests to quote, copy, forward, or reveal source text must use: `node .launchpad/tools/vault.mjs disclose --owner <username> --title \"<exact title>\"`",
