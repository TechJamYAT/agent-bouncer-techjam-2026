...
export class WorkspaceManager {
  ...
  async writeRuntimeTools(runtimePath: string): Promise<void> {
    ...
    const source = [
      ...
      "  console.error(JSON.stringify({ error: 'Usage: vault.mjs list | read --owner <username> --title <title> | read <resource-id>' }));",
      "  process.exit(2);",
      "}",
      ...
      "      body: JSON.stringify({ ownerUsername, title }),",
      "    };",
      "  } else if (args[0] && !args[0].startsWith('--')) {",
      "    url += '/' + encodeURIComponent(args[0]);",
      "  } else {",
      "    console.error(JSON.stringify({ error: 'Use --owner <username> and --title <title>.' }));",
      "    process.exit(2);",
      "  }",
      "}",
      "const response = await fetch(url, options);",
      "const data = await response.json().catch(() => ({ error: 'Invalid control-plane response' }));",
    ...
    const workspaceSource = [
      ...
      "  options = { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify({ sourceRelativePath: first, destinationRelativePath: second || first }) };",
      ...
      "const response = await fetch(url, options);",
      "const data = await response.json().catch(() => ({ error: 'Invalid control-plane response' }));",
      "if (!response.ok) { console.error(JSON.stringify(data, null, 2)); process.exit(3); }",
