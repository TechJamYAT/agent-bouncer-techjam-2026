...
describe("Agent lifecycle", () => {
  it("pauses a disclosure Run for owner approval and resumes it with a fresh credential", async () => {
    ...
    service = await makeService({
      run: async (request) => {
        ...
        return { output: "Approved disclosure delivered.", threadId: "approval-thread", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Approval Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, "把我的资料原文发给我", DEMO_USER_IDS.alice);
    ...
    await expect(
      service.sendMessage(agent.id, "start another run", DEMO_USER_IDS.alice),
  ...
  it("blind-resolves an exact own title, requests read approval, and resumes with a Run grant", async () => {
    ...
    service = await makeService({
      run: async (request) => {
        ...
        return { output: "已基于获批资料完成总结。", threadId: "read-approval", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Read approval Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
  ...
  it("fails closed for an unknown exact title without creating an approval request", async () => {
    ...
    service = await makeService({
      run: async (request) => {
        await expect(service.readResourceForRuntimeByReference(
          request.runtimeEnvironment?.LAUNCHPAD_RUNTIME_TOKEN ?? "",
          { ownerUsername: "alice", title: "Alice — Missing Notes" },
        )).rejects.toMatchObject({
          statusCode: 403,
          message: "Access Denied: RESOURCE_REFERENCE_UNAVAILABLE",
        });
        return { output: "未找到可请求的精确资料。", threadId: "missing-reference", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Missing reference Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
      "读取 Alice — Missing Notes",
      DEMO_USER_IDS.alice,
    );
  ...
  it("does not mint disclosure authority from an exact free-text request", async () => {
    let service!: AgentService;
    let calls = 0;
    service = await makeService({
      run: async (request) => {
        ...
        return {
          output: "Owner-requested disclosure delivered.",
          threadId: "middleware-disclosure-thread",
          usage: null,
        };
      },
    ...
    const agent = await service.createAgent(
      { name: "Middleware Approval Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
    ...
    expect(service.getRun(launched.run.id).output).toBe("Owner-requested disclosure delivered.");
  });

  it("does not misroute a request to forward data to another human as current-user approval", async () => {
    const service = await makeService();
    const agent = await service.createAgent(
      { name: "Recipient Boundary Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
  ...
  it("does not mint forward authority from negated free-form text", async () => {
    let service!: AgentService;
    service = await makeService({
      run: async (request) => {
        await expect(service.forwardResourceForRuntimeByReference(
          request.runtimeEnvironment?.LAUNCHPAD_RUNTIME_TOKEN ?? "",
          {
            ownerUsername: "alice",
            title: "Alice — Private Interview Notes",
        ...
        )).rejects.toMatchObject({ message: "Access Denied: HUMAN_FORWARD_INTENT_REQUIRED" });
        return { output: "没有执行被否定的转发。", threadId: "negated-forward", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Negation-safe Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
    ...
    expect(service.getHumanDirectMessages(DEMO_USER_IDS.alice, DEMO_USER_IDS.bob)).toHaveLength(0);
  });

  it.each([
    "不要把 Bob — Private Launch Notes 发给我",
    "Do not send Bob — Private Launch Notes to me",
  ])("does not create a denial evidence contract from a negated cross-owner sentence: %s", async (prompt) => {
    const service = await makeService();
    const agent = await service.createAgent(
      { name: "Negated cross-owner Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, prompt, DEMO_USER_IDS.alice);
  ...
  it("pauses an Agent-proposed forward in the conversation and resumes after owner approval", async () => {
    let service!: AgentService;
    const runtimeTokens: string[] = [];
    let calls = 0;
    service = await makeService({
      run: async (request) => {
        ...
        runtimeTokens.push(token);
        if (calls === 1) {
          const proposed = await service.requestForwardApprovalForRuntime(token, {
            ownerUsername: "alice",
            title: "Alice — Private Interview Notes",
            recipientUsername: "bob",
          });
          expect(proposed).toMatchObject({ pending: true, accessRequest: { action: "forward" } });
          return { output: "等待资料所有者决定。", threadId: "forward-approval", usage: null };
        }
        const forwarded = await service.forwardResourceForRuntimeByReference(token, {
          ownerUsername: "alice",
          title: "Alice — Private Interview Notes",
          recipientUsername: "bob",
        });
    ...
    const agent = await service.createAgent(
      { name: "Forward approval Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
    ...
    expect(request).toMatchObject({
      action: "forward",
      requesterHumanId: DEMO_USER_IDS.alice,
  ...
  it("supports catalog approval followed by a separate forward approval in one Run", async () => {
      ...
      run: async (request) => {
        ...
        if (calls <= 2) {
          if (calls === 1) {
            ...
            expect(proposed).toMatchObject({ pending: true, accessRequest: { action: "list" } });
            return { output: "等待目录授权。", threadId: "catalog-forward", usage: null };
          }
          ...
          expect(catalog).toMatchObject({
            ...
            resources: expect.arrayContaining([
              expect.objectContaining({ title: "Alice — Private Interview Notes" }),
            ]),
          });
          const proposed = await service.requestForwardApprovalForRuntime(token, {
            ownerUsername: "alice",
          ...
          expect(proposed).toMatchObject({ pending: true, accessRequest: { action: "forward" } });
          return { output: "已确认资料，等待外发授权。", threadId: "catalog-forward", usage: null };
        }
        const forwarded = await service.forwardResourceForRuntimeByReference(token, {
          ownerUsername: "alice",
          title: "Alice — Private Interview Notes",
        ...
        expect(forwarded.policy.reasonCode).toBe("USER_INTENT_BOUND_FORWARD");
        return { output: "审批后已转发。", threadId: "catalog-forward", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Catalog forward Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
    ...
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("waiting_for_approval");
    const listRequest = service.listAccessRequests(DEMO_USER_IDS.alice)
      .find((request) => request.runId === launched.run.id && request.action === "list")!;
    await service.resolveAccessRequest(DEMO_USER_IDS.alice, listRequest.id, "approve");
    await expect.poll(() => service.listAccessRequests(DEMO_USER_IDS.alice)
      .some((request) => request.runId === launched.run.id && request.action === "forward" && request.status === "pending"))
      .toBe(true);
    const forwardRequest = service.listAccessRequests(DEMO_USER_IDS.alice)
      .find((request) => request.runId === launched.run.id && request.action === "forward")!;
    await service.resolveAccessRequest(DEMO_USER_IDS.alice, forwardRequest.id, "approve");
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("completed");
    expect(calls).toBe(3);
    expect(service.getHumanDirectMessages(DEMO_USER_IDS.bob, DEMO_USER_IDS.alice).at(-1)?.content)
      .toContain("Alice — Private Interview Notes");
    const catalogAllows = service.listDecisions(DEMO_USER_IDS.alice).filter((decision) =>
      decision.runId === launched.run.id &&
      decision.action === "resource:list" &&
      decision.decision === "allow" &&
      decision.reasonCode === "PRIVATE_CATALOG_METADATA_APPROVED"
    );
    expect(catalogAllows).toHaveLength(1);
    expect(catalogAllows[0]?.requestEvidence?.responseStatus).toBe(200);
  });

  it("creates a real catalog approval when the Agent only claims that it requested one", async () => {
    let calls = 0;
    const service = await makeService({
      run: async (request) => {
        calls += 1;
        return {
          output: calls === 1
            ? "好，我发起你私有目录的元数据访问请求。"
            : "目录访问未获批准。",
          threadId: request.threadId ?? "prose-only-catalog",
          usage: null,
          toolEvents: [],
        };
      },
    ...
    const agent = await service.createAgent(
      { name: "Catalog evidence Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, "查看我的目录", DEMO_USER_IDS.alice);

    await expect.poll(() => service.getRun(launched.run.id).status).toBe("waiting_for_approval");
    const request = service.listAccessRequests(DEMO_USER_IDS.alice)[0]!;
    expect(request).toMatchObject({ action: "list", status: "pending", runId: launched.run.id });
    expect(service.listDecisions(DEMO_USER_IDS.alice)).toContainEqual(expect.objectContaining({
      runId: launched.run.id,
      action: "resource:list",
      decision: "deny",
      reasonCode: "PRIVATE_CATALOG_APPROVAL_REQUIRED",
      requestEvidence: expect.objectContaining({ source: "control_plane" }),
    }));
    await service.resolveAccessRequest(DEMO_USER_IDS.alice, request.id, "reject");
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("completed");
    expect(calls).toBe(2);
  });
  ...
  it.each([
    "不要查看我的目录",
    "Do not list my private catalog",
  ])("does not create a catalog approval for a negated request: %s", async (prompt) => {
    const service = await makeService();
    const agent = await service.createAgent(
      { name: "Negated catalog Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, prompt, DEMO_USER_IDS.alice);
  ...
  it("resumes exactly once when catalog approval arrives before the initial Agent turn exits", async () => {
    let service!: AgentService;
    let calls = 0;
    let announceRequest!: () => void;
    let releaseInitial!: () => void;
    const requestCreated = new Promise<void>((resolve) => { announceRequest = resolve; });
    const initialMayExit = new Promise<void>((resolve) => { releaseInitial = resolve; });
    service = await makeService({
      run: async (request) => {
        calls += 1;
        const catalog = await service.getPrivateResourceCatalogForRuntime(
        ...
        if (calls === 1) {
          ...
          announceRequest();
          await initialMayExit;
          return { output: "等待目录审批。", threadId: "fast-catalog-approval", usage: null };
        }
        expect(catalog).toMatchObject({
          visibility: "metadata_only",
          resources: expect.arrayContaining([
        ...
        return { output: "已读取获批目录。", threadId: "fast-catalog-approval", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Fast approval Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, "查看我的目录", DEMO_USER_IDS.alice);

    await requestCreated;
    expect(service.getRun(launched.run.id).status).toBe("waiting_for_approval");
    const request = service.listAccessRequests(DEMO_USER_IDS.alice)[0]!;
    await service.resolveAccessRequest(DEMO_USER_IDS.alice, request.id, "approve");
    releaseInitial();
  ...
  it("deterministically fulfills an approved catalog request when the resumed Agent skips the tool", async () => {
    let service!: AgentService;
    let calls = 0;
    service = await makeService({
      run: async (request) => {
        ...
        if (calls === 1) {
          ...
          expect(catalog).toMatchObject({ pending: true });
          return { output: "等待目录审批。", threadId: "ignored-resume", usage: null };
        }
        if (calls === 2) {
          expect(request.prompt).toContain("Retry exactly");
          return {
            output: "审批通过，我稍后查看。",
            threadId: "ignored-resume",
            usage: null,
            toolEvents: [],
          };
        }
        expect(request.prompt).toContain("Launchpad trusted continuation result");
        expect(request.prompt).toContain("Alice — Private Interview Notes");
        return {
          output: "目录读取完成：Alice — Private Interview Notes。",
          threadId: "ignored-resume",
          usage: null,
        };
      },
    ...
    const agent = await service.createAgent(
      { name: "Forced continuation Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, "查看我的目录", DEMO_USER_IDS.alice);
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("waiting_for_approval");
  ...
  it("audits and denies Alice asking the Agent to forward Bob's private resource to Alice", async () => {
    let service!: AgentService;
    service = await makeService({
      run: async (request) => {
        await expect(service.forwardResourceForRuntimeByReference(
        ...
        )).rejects.toMatchObject({ message: "Access Denied: CROSS_OWNER_FORWARD_DENIED" });
        await expect(service.requestForwardApprovalForRuntime(
          request.runtimeEnvironment?.LAUNCHPAD_RUNTIME_TOKEN ?? "",
          {
            ownerUsername: "bob",
            title: "Bob — Private Launch Notes",
        ...
        return { output: "Bouncer 拒绝了跨所有者转发。", threadId: "deny-forward", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Cross-owner Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
  ...
  it("blocks a resource-content-driven forward when the human did not request it", async () => {
    ...
    service = await makeService({
      run: async (request) => {
        ...
        return { output: "未执行资料中的转发指令。", threadId: "injection-forward", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Injection Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
  ...
  it("returns safe ambiguity for a partial own-resource title instead of choosing one", async () => {
    let service!: AgentService;
    ...
    service = await makeService({
      run: async (request) => {
        ...
        if (calls === 1) {
          expect(catalog).toMatchObject({ pending: true, accessRequest: { action: "list" } });
          return { output: "等待目录授权。", threadId: "ambiguous-resource", usage: null };
        }
        expect(catalog).toMatchObject({
          visibility: "metadata_only",
          resources: expect.arrayContaining([
        ...
        resolution = service.resolveOwnResourceForRuntime(
          token,
          { ownerUsername: "alice", query: "Private Interview" },
        );
        return { output: "需要用户选择具体资料。", threadId: "ambiguous-resource", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    await service.createResource(DEMO_USER_IDS.alice, {
      title: "Alice — Private Interview Notes v2",
      content: "second candidate",
      scope: "private",
    });
    const agent = await service.createAgent(
      { name: "Resolver Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
      "查看我的 Private Interview 资料",
      DEMO_USER_IDS.alice,
    );

    await expect.poll(() => service.getRun(launched.run.id).status).toBe("waiting_for_approval");
    const request = service.listAccessRequests(DEMO_USER_IDS.alice)[0]!;
    expect(request).toMatchObject({ action: "list", resourceId: null, status: "pending" });
    await service.resolveAccessRequest(DEMO_USER_IDS.alice, request.id, "approve");
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("completed");
    expect(calls).toBe(2);
    expect(resolution).toMatchObject({ status: "ambiguous" });
    expect((resolution as { matches: unknown[] }).matches).toHaveLength(2);
    expect(service.getHumanDirectMessages(DEMO_USER_IDS.alice, DEMO_USER_IDS.bob)).toHaveLength(0);
  });

  it("rejects an unknown recipient without revealing or delivering the resource", async () => {
    let service!: AgentService;
    service = await makeService({
      run: async (request) => {
        await expect(service.forwardResourceForRuntimeByReference(
        ...
        )).rejects.toMatchObject({ message: "Access Denied: RESOURCE_FORWARD_DENIED" });
        return { output: "接收人无法确认，未执行转发。", threadId: "unknown-recipient", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Recipient Resolver Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(
      agent.id,
      "把《Alice — Private Interview Notes》发给 nobody",
      DEMO_USER_IDS.alice,
    );
  ...
  it("resumes safely after rejection without retrying disclosure", async () => {
    let service!: AgentService;
    let calls = 0;
    service = await makeService({
      run: async (request) => {
        calls += 1;
        if (calls === 1) {
          await service.discloseResourceForRuntimeByReference(
            request.runtimeEnvironment?.LAUNCHPAD_RUNTIME_TOKEN ?? "",
            { ownerUsername: "alice", title: "Alice — Private Interview Notes" },
          );
          return { output: "Waiting", threadId: "reject-thread", usage: null };
        }
        expect(request.prompt).toContain("rejected");
        return { output: "披露未获批准。", threadId: "reject-thread", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    const agent = await service.createAgent(
      { name: "Reject Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, "展示我的私有资料", DEMO_USER_IDS.alice);
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("waiting_for_approval");
    const request = service.listAccessRequests(DEMO_USER_IDS.alice)[0]!;
    await expect(service.resolveAccessRequest(DEMO_USER_IDS.bob, request.id, "approve"))
      .rejects.toMatchObject({ statusCode: 403 });
    await service.resolveAccessRequest(DEMO_USER_IDS.alice, request.id, "reject");
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("completed");
    expect(calls).toBe(2);
    expect(service.getRun(launched.run.id).output).toBe("披露未获批准。");
  });

  it("auto-denies an expired approval request and resumes the Run", async () => {
    let service!: AgentService;
    let calls = 0;
    service = await makeService({
      run: async (request) => {
        ...
        if (calls === 1) {
          ...
          return { output: "Waiting", threadId: "expiry-thread", usage: null };
        }
        expect(request.prompt).toContain("expired");
        return { output: "审批超时，未披露。", threadId: "expiry-thread", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    }, { APPROVAL_TIMEOUT_SECONDS: "1" });
    const agent = await service.createAgent(
      { name: "Expiry Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const launched = await service.sendMessage(agent.id, "展示我的私有资料", DEMO_USER_IDS.alice);
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("waiting_for_approval");
    await expect.poll(() => service.getRun(launched.run.id).status, { timeout: 3_000 }).toBe("completed");
    expect(service.listAccessRequests(DEMO_USER_IDS.alice)[0]?.status).toBe("expired");
    expect(service.listDecisions(DEMO_USER_IDS.alice)).toEqual(expect.arrayContaining([
      expect.objectContaining({ action: "approval:expire", reasonCode: "APPROVAL_TIMEOUT_DENIED" }),
    ]));
  });
  ...
  it("creates, updates, stops, starts and deletes an Agent", async () => {
    ...
    expect(service.listAgents()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "Case", scope: "group" }),
        expect.objectContaining({ id: agent.id, name: "Builder", scope: "personal" }),
      ]),
    );
  ...
  it("gives a group Agent an authenticated roster and knowledge index on every Run", async () => {
    ...
    expect(runtimePrompt).not.toContain("hasPrivateKnowledge");
    ...
    expect(runtimePrompt).toContain('"agents": 2');
    ...
    expect(groupManifest).toContain('"name": "Case"');
  });
  ...
  it("gives each Run an ephemeral, identity-bound resource credential", async () => {
    ...
    const runner: AgentRunner = {
      run: async (request) => {
        ...
        expect(catalog).not.toHaveProperty("privateKnowledgeOwners");
        expect(JSON.stringify(catalog)).not.toContain("Bob — Private Launch Notes");
...
describe("Multi-Agent coordination Runtime", () => {
  ...
  it("lets Case process Bob's task data for Alice without disclosing Bob's source", async () => {
    ...
    const task = await service.createCoordinationSession(DEMO_USER_IDS.alice, {
      ...
      middlewareEvidenceRequirements: [
        { action: "resource:process", decision: "allow" },
        { action: "resource:disclose", decision: "deny" },
      ],
    });
    ...
    expect(launched.run.prompt).toContain("Server-enforced middleware evidence contract");
  ...
  it("fails and exposes a retry when an Agent skips required middleware actions", async () => {
    const service = await makeService({
      run: async () => ({
        output: "I reviewed the title without calling the protected tool.",
        threadId: "missing-middleware-evidence-thread",
        usage: null,
        toolEvents: [],
      }),
      cancel: async () => false,
    ...
    const caseAgent = await service.createAgent(
      { name: "Case", scope: "group", groupId: DEMO_GROUP_IDS.alpha },
      DEMO_USER_IDS.alice,
    );
    const task = await service.createCoordinationSession(DEMO_USER_IDS.alice, {
      groupId: DEMO_GROUP_IDS.alpha,
      ...
      objective: "Assess Bob's notes and request disclosure.",
      participantAgentIds: [caseAgent.id],
      middlewareEvidenceRequirements: [
        { action: "resource:process", decision: "allow" },
        { action: "resource:disclose", decision: "deny" },
    ...
    await service.createGrant(DEMO_USER_IDS.bob, {
      agentId: caseAgent.id,
      resourceId: DEMO_RESOURCE_IDS.bobPrivate,
      action: "process",
      duration: "task",
    ...
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("failed");
    const failedRun = service.getRun(launched.run.id);
    expect(failedRun.middlewareEvidenceStatus).toBe("missing");
    expect(failedRun.error).toContain("resource:process=allow");
    expect(failedRun.error).toContain("resource:disclose=deny");
    const failedTask = service.getCoordinationSession(DEMO_USER_IDS.alice, task.session.id);
    expect(failedTask.session.status).toBe("failed");
    const failedStep = failedTask.steps.find((step) => step.status === "failed");
    expect(failedStep).toBeDefined();

    const retry = await service.retryCoordinationStep(
      DEMO_USER_IDS.alice,
      task.session.id,
      failedStep!.id,
      failedTask.session.version,
    );
    expect(retry.session.status).toBe("waiting_for_human");
    expect(retry.steps.find((step) => step.id === failedStep!.id)?.status).toBe("pending");
  });

  it("does not satisfy an evidence contract with a decision for another resource", async () => {
    let service!: AgentService;
    service = await makeService({
      run: async (request) => {
        const token = request.runtimeEnvironment?.LAUNCHPAD_RUNTIME_TOKEN ?? "";
        await service.processResourceForRuntimeByReference(token, {
          ownerUsername: "alice",
          title: "Alice — Private Interview Notes",
          operation: "launch-risk-check",
        });
        await expect(service.discloseResourceForRuntimeByReference(token, {
          ownerUsername: "bob",
          title: "Bob — Private Launch Notes",
        })).rejects.toMatchObject({ statusCode: 403 });
        return { output: "Used the wrong source.", threadId: "wrong-source", usage: null };
      },
      cancel: async () => false,
      isAvailable: async () => true,
    });
    ...
    const task = await service.createCoordinationSession(DEMO_USER_IDS.alice, {
      ...
      kind: "task",
      mode: "manual",
      title: "Resource-bound evidence",
      objective: "Process Bob's notes and request disclosure.",
      participantAgentIds: [agent.id],
      middlewareEvidenceRequirements: [
        { action: "resource:process", decision: "allow" },
        { action: "resource:disclose", decision: "deny" },
    ...
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("failed");
    expect(service.getRun(launched.run.id).error).toContain("resource:process=allow");
  });
  ...
  it("keeps a human group chat usable before Agents join and lets a member request a reply", async () => {
    ...
    let chat = await service.createCoordinationSession(DEMO_USER_IDS.bob, {
      groupId: DEMO_GROUP_IDS.beta,
      kind: "group_chat",
      mode: "manual",
      title: "Beta group chat",
      participantAgentIds: [],
    });
    expect(chat.session.participantAgentIds).toEqual([]);
    expect(
      service.listGroups(DEMO_USER_IDS.bob).find((group) => group.id === DEMO_GROUP_IDS.beta)
        ?.lastActivityAt,
    ).toBeNull();
    ...
    const agent = await service.createAgent(
      { name: "Reviewer", scope: "group", groupId: DEMO_GROUP_IDS.beta },
      DEMO_USER_IDS.bob,
    );
    const launched = await service.advanceCoordinationSession(
      DEMO_USER_IDS.david,
    ...
    await expect.poll(() => service.getRun(launched.run.id).status).toBe("completed");
    expect(initiatingHumanId).toBe(DEMO_USER_IDS.david);
    expect(contextAtRun).toContain('"username": "david"');
    chat = service.getCoordinationSession(DEMO_USER_IDS.david, chat.session.id);
