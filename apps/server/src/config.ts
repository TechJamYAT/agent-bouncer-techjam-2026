...
const envSchema = z.object({
  ...
  APPROVAL_TIMEOUT_SECONDS: z.coerce.number().int().min(1).max(86_400).default(300),
...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    approvalTimeoutMs: env.APPROVAL_TIMEOUT_SECONDS * 1_000,
...
export async function writeCodexConfig(config: AppConfig): Promise<void> {
  ...
  const toml = [
    "# Generated by Volc Agent Launchpad from environment or in-app Runtime settings.",
